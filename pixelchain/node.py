...
class Node:
    ...
    async def start(self):
        ...
        if self.blockchain.load_state(self.config.data_dir):
            logger.info("Resumed from saved state — epoch %s, %d closures",
                        self.blockchain.current_epoch, len(self.blockchain.closures))
        ...
        logger.info("Node started on port %d (WS: %d)", self.config.tcp_port, self.config.ws_port)
    ...
    async def _handle_hello(self, peer: PeerInfo, msg: dict):
        ...
        if peer.best_work > self.blockchain.total_work:
            logger.info("Peer %s has more work (%d vs %d), syncing",
                        peer.address, peer.best_work, self.blockchain.total_work)
            await self.network.send_to_peer(peer.address, {
                'type': 'get_closures',
                'from_epoch': 'a',
            })
        elif peer.best_closure and peer.best_closure != self.blockchain.current_epoch:
            # Peer might need sync from us, or we might need catch-up
            pass
    ...
    async def _handle_reject(self, peer: PeerInfo, msg: dict):
        ...
        logger.debug("Reject from %s: reason=%s hash=%s",
                      peer.address, msg.get('reason'), msg.get('hash'))
    ...
    def _notify_canvas_update(self, x: int, y: int, r: int, g: int, b: int):
        ...
        if self._loop is not None:
            self._loop.call_soon_threadsafe(
                lambda: self._loop.create_task(self._ws_broadcast({
                    'type': 'canvas_update',
                    'x': x, 'y': y, 'color': color,
                }))
            )
