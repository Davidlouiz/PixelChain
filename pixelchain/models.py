...
class Pixel:
    ...
    prev_pixel_hash: Optional[bytes]  # hash of previous pixel at same (x,y), None if first
    nonce: int = 0
    ...
    def verify(self, difficulty_bits: int) -> bool:
        ...
        return expected == self.hash and hash_meets_difficulty(self.hash, difficulty_bits)
    ...
    def from_dict(cls, d: dict) -> 'Pixel':
        ...
        prev_hash = bytes.fromhex(d['prev_pixel_hash']) if 'prev_pixel_hash' in d and d['prev_pixel_hash'] else None
        px = cls(
            x=d['x'],
            y=d['y'],
            r=r, g=g, b=b,
...
class ClosureBlock:
    ...
    nonce: int = 0
    hash: bytes = field(default=b'\x00' * 32, repr=False)
    timestamp: float = 0.0  # time this closure was created (unix ts), for difficulty calc
